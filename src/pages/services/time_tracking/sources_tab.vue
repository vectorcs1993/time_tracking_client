...
function update() {
  rows.value.length = 0;
  getQuery(`${host()}/services/genprice/TimeTrackingSource`).then((resp) => {
...
function add() {
  ...
  postQuery(`${host()}/services/genprice/TimeTrackingSource/create`, query)
    .then((res) => {
...
function change() {
  const query = { name: modelInput.value.name };
  postQuery(`${host()}/services/genprice/TimeTrackingSource/update/${selected.value[0].id}`, query)
    .then((res) => {
...
function remove() {
  props.showConfirm(`Удалить источник поступления задач ${selected.value[0].name}?`, () => {
    getQuery(`${host()}/services/genprice/TimeTrackingSource/delete/${selected.value[0].id}`).then(() => {
      update();
    });
  });
}
