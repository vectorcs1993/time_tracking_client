    ...
    <q-table v-if="curentConfig" ref="table" dense
      ...
      <template v-slot:top>
        <q-card-actions class="row q-gutter-sm full-width items-center">
          ...
          <PPSimpleSelect label="Сотрудник" v-model="inputFilter.user" :options="inputFilter.usersOnlyBranch"
            @update:model-value="updateInputFilter" :dark="props.dark" style="width: 200px;" />
          <!-- Фильтр тип работы -->
          <PPSimpleSelect v-if="inputFilter.on" label="Тип работы" v-model="inputFilter.type_work"
          ...
          <q-select v-if="inputFilter.on" :disable="inputFilter.type_work ? inputFilter.type_work.id === -1 : false"
            class="text-size" square :standout="`${props.dark ? 'bg-grey text-white' : 'bg-green text-white'}`"
            :dark="props.dark" dense options-selected-class="text-grey" label="Целевой объект"
            :options="inputFilter.activities" option-value="id" popup-content-class="text-size"
            v-model="inputFilter.activity" @update:model-value="() => {
              updateFilterTypeWork();
              updateInputFilter();
          ...
          <q-select class="text-size" square :standout="`${props.dark ? 'bg-grey text-white' : 'bg-green text-white'}`"
            ...
            :options="type_period" option-value="id" v-model="inputFilter.period"
            @update:model-value="updateInputFilter" popup-content-class="text-size">
            <template v-slot:selected-item="scope">
              {{ scope.opt.name }}
            </template>
            <template v-slot:option="scope">
          ...
          <div class="row q-gutter-sm items-center q-ma-sm">
            <span>
              от
            </span>
            <q-input class="text-size" type="date" :dark="props.dark" dense square
              :standout="`${props.dark ? 'bg-grey text-white' : 'bg-green text-white'}`"
              :disable="inputFilter.period.id !== 0" v-model="inputFilter.dateStart"
            ...
            <q-input class="text-size" type="date" :dark="props.dark" dense square
              :standout="`${props.dark ? 'bg-grey text-white' : 'bg-green text-white'}`"
              :disable="inputFilter.period.id !== 0" v-model="inputFilter.dateFinish"
              @update:model-value="updateInputFilter">
            </q-input>
          </div>
          <!-- Поиск -->
      ...
      <template v-slot:body-cell="props">
        <q-td :props="props" class="no-pa-ma" :style="`background-color: ${getCustomStyle(props.row, props.col.name)}`">
          <q-input class="text-size " input-style="text-align: center;"
            v-if="(props.col.type == 'text' || props.col.type == 'textarea') && isSelect(props.row.id) && isAllowEdit(props.row.id, props.col.name, true)"
            type="text" :dark="props.dark" dense square
            :standout="`${props.dark ? 'bg-grey text-white' : 'bg-green text-white'}`" autogrow
            v-model.text="props.row[props.col.name]" hide-bottom-space hide-hint @keydown="(event) => {
          ...
          <q-input class="text-size" input-style="text-align: center;"
            v-else-if="props.col.type == 'number' && isSelect(props.row.id) && isAllowEdit(props.row.id, props.col.name, true)"
            :standout="`${props.dark ? 'bg-grey text-white' : 'bg-green text-white'}`" type="text" :dark="props.dark"
            dense square v-model.text="props.row[props.col.name]" @keydown="(event) => {
              handleKeydown(event, props.col.name, props.row[props.col.name]);
            }" @blur="(val) => {
              save(props.col.name, props.row[props.col.name]);
            }">
          ...
          <q-select dense square :standout="`${props.dark ? 'bg-grey text-white' : 'bg-green text-white'}`"
            :disable="props.col.disable" class="text-size q-pa-none q-ma-none"
            v-else-if="props.col.type == 'selector' && isSelect(props.row.id) && isAllowEdit(props.row.id, props.col.name, true)"
            :dark="props.dark" options-selected-class="text-grey" :options="props.col.options(props.row)"
            option-value="id" v-model.number="props.row[props.col.name]" input-class="q-pa-none q-ma-none"
            @update:model-value="(val) => {
              if (props.col.name === 'type_work') {
                updateTypeWork(props.row);
              } else {
                save(props.col.name, val.id);
              }
            }" input-style="text-align: center;" popup-content-class="text-size">
            <template v-slot:selected-item="scope">
              <div class="row fit justify-center items-center" style="white-space: pre-line;">
                {{ scope.opt.name }}
              </div>
            </template>
            <template v-slot:option="scope">
              <q-item class="row fit justify-center items-center" style="white-space: pre-line;"
